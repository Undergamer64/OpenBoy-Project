...
bool IF_LD_r_r::IsValid(uint8_t opcode) {
	return (opcode & 0b11000000) == 0b01000000 && (opcode & 0b00111111) != 0b00110110;
	// all load r,r except LD (HL), (HL), which is HALT instruction
}
...
#pragma region IF_ARR
bool IF_ARR::IsValid(uint8_t opcode) {
	return ((opcode & 0b11100000) == 0b10000000 ||
	        (opcode & 0b11100000) == 0b10100000) ||
	       (opcode & 0b11000111) == 0b11000110;
}

bool IF_ARR::Tick(uint8_t opcode, MMU &mmu, Registers &registers) {
	switch (m_currentCycle) {
	case 0:
		...
		m_currentCycle++;
	//fallthrough if not HL or immediate
	case 1:
		uint8_t _res = 0;
...
#pragma region Arithmetique
			switch (opcode & 0b00011000) {
			case 0b00000000:
				_res = registers.m_registers[6] + m_8bitRegister + (registers.m_registers[7] & 0b00000001);
#pragma region Flags
#pragma region Flag_S
				registers.m_registers[7] &= ~0b10000000; //Flag s (negative)
#pragma endregion
...
#pragma region Flag_C
				if (m_8bitRegister > _res - (registers.m_registers[7] & 0b00000001) || registers.m_registers
				    [6] > _res - (registers.m_registers[7] & 0b00000001))
				//c (Carry for the 7 bit) ONLY FOR ADDS, NOT FOR SUBS !!!
				{
...
#pragma region Flag_H
				if ((m_8bitRegister & 0b00001111) > (
					    (_res - (registers.m_registers[7] & 0b00000001)) & 0b00001111) || (
					    registers.m_registers[6] & 0b00001111) > (
					    (_res - (registers.m_registers[7] & 0b00000001)) & 0b00001111))
				//Flag h (half-carry) same method as Flag C but with mask
				{
...
#pragma endregion
				break;
			case 0b00001000:
				_res = registers.m_registers[6] + m_8bitRegister;
#pragma region Flags
#pragma region Flag_S
				registers.m_registers[7] &= ~0b10000000; //Flag s (negative)
#pragma endregion
...
#pragma endregion
#pragma region Flag_C
				if (m_8bitRegister > _res || registers.m_registers[6] > _res)
				//c (Carry for the 7 bit) ONLY FOR ADDS, NOT FOR SUBS !!!
				{
					registers.m_registers[7] |= 0b00000001;
				} else {
...
#pragma endregion
#pragma region Flag_H
				if ((m_8bitRegister & 0b00001111) > ((_res) & 0b00001111) || (
					    registers.m_registers[6] & 0b00001111) > ((_res) & 0b00001111))
				//Flag h (half-carry) same method as Flag C but with mask
				{
					registers.m_registers[7] |= 0b00010000;
				} else {
...
#pragma region Flag_C
				if (m_8bitRegister < _res + (registers.m_registers[7] & 0b00000001) || registers.m_registers
				    [6] < _res + (registers.m_registers[7] & 0b00000001))
				//c (Borrow for the 7 bit) ONLY FOR SUBS, NOT FOR ADDS !!!
				{
					registers.m_registers[7] |= 0b00000001;
				} else {
					registers.m_registers[7] &= ~0b00000001;
...
#pragma region Flag_H
				if ((m_8bitRegister & 0b00001111) < (
					    ...
					    registers.m_registers[6] & 0b00001111) < (
					    (_res + (registers.m_registers[7] & 0b00000001)) & 0b00001111))
				//Flag h (half-borrow) same method as Flag c but with mask
				{
					registers.m_registers[7] |= 0b00010000;
				} else {
					registers.m_registers[7] &= ~0b00010000;
...
#pragma region Flag_C
				if (m_8bitRegister < _res || registers.m_registers[6] < _res)
				//Flag c (Borrow for the 7 bit) ONLY FOR SUBS, NOT FOR ADDS !!!
				{
					registers.m_registers[7] |= 0b00000001;
				} else {
					registers.m_registers[7] &= ~0b00000001;
...
bool IF_ADD_HL::Tick(uint8_t opcode, MMU &mmu, Registers &registers) {
	...
	switch (m_currentCycle) {
	case 0:
		m_8bitRegister = registers.m_registers[5]; //L

		switch (opcode & 0b00110000) {
		case 0b00000000: {
			reg = registers.m_registers[1]; // +C
			break;
		}
		case 0b00010000: {
			reg = registers.m_registers[3]; // +E
			break;
		}
		case 0b00100000: {
			reg = registers.m_registers[5]; // +L
			break;
		}
		case 0b00110000: {
			reg = registers.SP & 0x00FF; // +SP (low byte)
			break;
		}
		}
...
#pragma endregion
#pragma region Flag_C
		if (reg > m_8bitRegister || registers.m_registers[5] > m_8bitRegister) //c (Carry for the 7 bit) ONLY FOR ADDS, NOT FOR SUBS !!!
		{
			registers.m_registers[7] |= 0b00000001;
		} else {
			registers.m_registers[7] &= ~0b00000001;
...
#pragma endregion
#pragma region Flag_H
		if ((reg & 0b00001111) > (m_8bitRegister & 0b00001111) ||
			(registers.m_registers[5] & 0b00001111) > (m_8bitRegister & 0b00001111)) //Flag h (half-carry) same method as Flag C but with mask
		{
			registers.m_registers[7] |= 0b00010000;
		} else {
			registers.m_registers[7] &= ~0b00010000;
...
#pragma endregion
		...
		registers.m_registers[5] = m_8bitRegister;
		break;
	case 1:
		m_8bitRegister = registers.m_registers[4]; //H

		switch (opcode & 0b00110000) {
		case 0b00000000: {
			reg = registers.m_registers[0]; // +B
			break;
		}
		case 0b00010000: {
			reg = registers.m_registers[2]; // +D
			break;
		}
		case 0b00100000: {
			reg = registers.m_registers[4]; // +H
			break;
		}
		case 0b00110000: {
			reg = (registers.SP & 0xFF00) >> 8; // +SP (high byte)
			break;
		}
		}
...
#pragma endregion
#pragma region Flag_C
		if (reg > m_8bitRegister || registers.m_registers[4] > m_8bitRegister) //c (Carry for the 7 bit) ONLY FOR ADDS, NOT FOR SUBS !!!
		{
			registers.m_registers[7] |= 0b00000001;
		} else {
			registers.m_registers[7] &= ~0b00000001;
...
#pragma region Flag_H
		if ((reg & 0b00001111) > (m_8bitRegister & 0b00001111) ||
			(registers.m_registers[4] & 0b00001111) > (m_8bitRegister & 0b00001111)) //Flag h (half-carry) same method as Flag C but with mask
		{
			registers.m_registers[7] |= 0b00010000;
		} else {
			registers.m_registers[7] &= ~0b00010000;
...
#pragma endregion
		...
		registers.m_registers[4] = m_8bitRegister;
...
#pragma region IF_CPL

bool IF_CPL::IsValid(uint8_t opcode) {
	return opcode == 0x2F;
}

bool IF_CPL::Tick(uint8_t opcode, MMU &mmu, Registers &registers) {
	registers.m_registers[6] = ~registers.m_registers[6];
...
#pragma endregion
#pragma region Flag_H
	registers.m_registers[7] |= 0b00010000; //Flag h
#pragma endregion
#pragma endregion
...
bool IF_FLOW_JR::Tick(uint8_t opcode, MMU &mmu, Registers &registers) {
	...
	case 0: //Fetch wait
		...
		break;
	case 1:
		if ((opcode & 0b11100111) == 0b00100000 && !m_condition) //If JR with condition
		{
			m_currentCycle = 0;
			return true;
		}
		...
		registers.PC = static_cast<uint16_t>(
			static_cast<int32_t>(registers.PC) + static_cast<int8_t>(m_8bitRegister)
		);
...
bool IF_FLOW_JP::IsValid(uint8_t opcode) {
	return (opcode & 0b11111111) == 0b11000011 || (opcode & 0b11111111) == 0b11101001 || (opcode & 0b11100111) ==
	       0b11000010;
}
...
bool IF_FLOW_CALL::Tick(uint8_t opcode, MMU &mmu, Registers &registers) {
	...
	case 2:
		...
		if ((opcode & 0b11000111) == 0b11000100 && !m_condition) {
			m_currentCycle = 0;
			return true;
		}
...
bool IF_FLOW_RET::Tick(uint8_t opcode, MMU &mmu, Registers &registers) {
	...
	case 2:
		...
		if (registers.SP < 0xFF80) {
			std::cout << "Stack Pointer out of bounds on POP instruction" << std::endl;
		}
		break;
	case 3:
...
bool IF_ROTATE::Tick(uint8_t opcode, MMU &mmu, Registers &registers) {
	...
	case 0b00000000: // Left
		...
		} else // Circular
		{
			...
			registers.m_registers[6] = (registers.m_registers[6] << 1) + (
				                           (registers.m_registers[6] & 0b10000000) >> 7);
		}
		break;
	case 0b00001000: // Right
		...
		} else // Circular
		{
			...
			registers.m_registers[7] |= (registers.m_registers[6] & 0b00000001); //Flags C
			registers.m_registers[6] = (registers.m_registers[6] >> 1) + (
				                           (registers.m_registers[6] & 0b00000001) << 7);
		}
		break;
	}
...
bool IF_CB_RES_SET::Tick(uint8_t opcode, MMU &mmu, Registers &registers) {
	...
	case 1:
		if ((opcode & 0b01000000) == 0b01000000) {
			m_8bitRegister |= (0b00000001 << ((opcode & 0b00111000) >> 3));
		} else {
			m_8bitRegister &= ~(0b00000001 << ((opcode & 0b00111000) >> 3));
		}
		...
		if ((opcode & 0b00000111) == 0b0110) //If HL
		{
			mmu.Write(HL, m_8bitRegister);
			break;
		}

		registers.m_registers[(opcode & 0b00000111)] = m_8bitRegister;
...
bool IF_INC_DEC::Tick(uint8_t opcode, MMU &mmu, Registers &registers) {
	...
	case 0: // Increase/Decrease directly register or get HL value
		...
		if ((opcode & 0b11000110) == 0b00000100 && (opcode & 0b00111000) != 0b00110000)
		// using 8 bit registers and not HL
		{
...
#pragma region Flag_H
			if (D == 0) {
				if (((m_8bitRegister - _res) & 0b00001111) > (m_8bitRegister & 0b00001111))
				//Flag h (half-carry) same method as Flag C but with mask
				{
					registers.m_registers[7] |= 0b00010000;
				} else {
...
bool IF_POP::IsValid(uint8_t opcode) {
	return (opcode & 0b11001111) == 0b11000001;
}
...
bool IF_PUSH::IsValid(uint8_t opcode) {
	return (opcode & 0b11001111) == 0b11000101;
}
